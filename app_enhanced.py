...
def api_generate_merged_pdf():
    ...
    try:
        ...
        print(f"  conductor_path: {conductor_path}")
        print(f"  sheath_path: {sheath_path}")
        print(f"  datasheet_path: {datasheet_path}")
        
        # Check if files exist
        if conductor_path:
            print(f"  conductor file exists: {os.path.exists(conductor_path)}")
        if sheath_path:
            print(f"  sheath file exists: {os.path.exists(sheath_path)}")
        if datasheet_path:
            print(f"  datasheet file exists: {os.path.exists(datasheet_path)}")
        
        # Always try to merge all available reports
        merger = PdfMerger()
        has_content = False
        
        # Add conductor report if available (first)
        if conductor_path and os.path.exists(conductor_path):
            merger.append(conductor_path)
            has_content = True
            print("Added conductor report to merge")
        
        # Add sheath report if available (second)
        if sheath_path and os.path.exists(sheath_path):
            merger.append(sheath_path)
            has_content = True
            print("Added sheath report to merge")
